...
def ensure_dependencies():
    ...
    # Try to ensure aria2 and ffmpeg are available (best-effort on Debian/Ubuntu)
    for pkg in ("aria2", "ffmpeg"):
...
def download_video(url: str, dest: Path) -> Path:
    ...
    # Find the newest file in the destination folder
...
def process_url(url: str, dest: Path, series_slug: str | None = None, year: str = "2024") -> None:
    ...
    if not series_slug:
        # Fallback: try to guess from the last path component without the episode number
        # e.g. /hentai/gibo-no-toiki-1  →  gibo-no-toiki
        slug_part = url.rstrip("/").split("/")[-1]
        series_slug = re.sub(r"-\d+$", "", slug_part)

    # External subtitle source used by the original script
    # Format: https://oppai-str.shoujo-h.org/{year}/{Series.Name}/E{ep:02}/eng.ass
    # Note: series name must use dots instead of hyphens/spaces (user must supply correct slug)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Series slug for external subtitles, e.g. 'Gibo.no.Toiki' "
             "(dots instead of spaces/hyphens). If omitted, a simple guess is made.",
    )
    parser.add_argument(
        "--year",
        default="2024",
    ...
    for i, url in enumerate(tqdm(urls, desc="Overall", unit="video"), start=1):
        tqdm.write(f"\n🔄 [{i}/{len(urls)}] {url}")
        try:
            process_url(url, dest, series_slug=args.series_slug, year=args.year)
        except Exception as e:
